...
for i = 1:length(sus_l)
    ...
    if any(nonzero_suspicous{i})
        ...
        % Plot higer horizontal connector
        sus_x = linspace(sus_b(i), sus_c(i), 50);
        sus_y = ones(size(sus_x));
        ps = plot(sus_x, sus_y, '-y', 'LineWidth', 2, 'DisplayName', 'Suspicious');

        % Plot lower horizontal connector
        sus_xl1 = linspace(min(min(min(nonzero_abnormal{i},norm_g(i))),min(sus_xu)), sus_a(i), 50);
        sus_yl1 = zeros(size(sus_xl1));
        plot(sus_xl1, sus_yl1, '-y', 'LineWidth', 2);
        sus_xl2 = linspace(sus_d(i), (max(max(nonzero_normal{i}),max(sus_xd))), 50);
        sus_yl2 = zeros(size(sus_xl2));
        plot(sus_xl2, sus_yl2, '-y', 'LineWidth', 2);
    end
    ...
    if any(nonzero_normal{i})
        ...
        % Plot higher horizontal connector
        norm_x = linspace(norm_h(i), max(nonzero_normal{i}), 50);
        norm_y = ones(size(norm_x));
        pn = plot(norm_x, norm_y, '-g', 'LineWidth', 2, 'DisplayName', 'Normal');

        % Plot lower horizontal connector
        norm_xl = linspace(min(min(min(nonzero_abnormal{i}),min(norm_xu)),min(sus_xu)), norm_g(i), 50);
        norm_yl = zeros(size(norm_xl));
        plot(norm_xl, norm_yl, '-g', 'LineWidth', 2);
    end
    ...
    if any(nonzero_abnormal{i})
        ...
        pa = plot(abn_x, abn_y, '-r', 'LineWidth', 2, 'DisplayName', 'Abnormal');

        % Plot lower horizontal connector
        abn_xl = linspace(abn_f(i), max(max(max(nonzero_normal{i}),max(sus_xd)),max(abn_xd)), 50);
        abn_yl = zeros(size(abn_xl));
        plot(abn_xl, abn_yl, '-r', 'LineWidth', 2);        
    end
